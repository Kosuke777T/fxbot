...
T-51-4 Virtual BT タブ：CSV選択UX改善（初期フォルダ固定＋前回選択の永続化）

■ 目的

CSV選択の手間と事故（選び直し・選択ミス）を減らし、
仮想実行バックテストの再現性と操作安定性を高める。

■ 実施内容（事実）

QSettings を用いて CSV パスを永続化

CSV選択ダイアログの初期フォルダを D:\fxbot\data に固定

起動時に前回選択CSVを自動復元（存在チェック付き）

ファイル消失時は設定クリアして安全にフォールバック
...
_restore_last_csv_path()（GUI内部完結）
...
Qt標準機構（QSettings）のみ使用

責務境界（gui / services / core）厳守

既存API・既存挙動を破壊しない

推測ではなく Path.exists による観測ベース制御

■ 挙動の変化

変わった点

CSV選択ダイアログの初期位置が常に D:\fxbot\data

前回選択したCSVが次回起動時に自動復元される
...
python -X utf8 -m py_compile app/gui/virtual_bt_tab.py

GUI起動 → CSV選択 → 終了 → 再起動

CSV削除後の起動でエラーが出ないことを確認
