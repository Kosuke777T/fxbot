...
class VirtualBTTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        profile_row.addWidget(QLabel("閾値上書き:"))
        self.threshold_override_check = QCheckBox("有効")
        self.threshold_override_check.setChecked(False)
        profile_row.addWidget(self.threshold_override_check)
        self.threshold_spin = QDoubleSpinBox()
        self.threshold_spin.setRange(0.30, 0.70)
        self.threshold_spin.setSingleStep(0.01)
        self.threshold_spin.setDecimals(2)
        self.threshold_spin.setValue(0.45)
        profile_row.addWidget(self.threshold_spin)
        ...
        self._add_stat_row("全体", "スキップ数", "n_skips", "0")
        self._add_stat_row("全体", "使用閾値", "used_threshold", "—")
        self._add_stat_row("全体", "閾値ソース", "threshold_source", "—")
    ...
    def _on_start_clicked(self):
        ...
        try:
            ...
            if self.threshold_override_check.isChecked():
                threshold_override = float(self.threshold_spin.value())
    ...
    def _update_stats(self):
        ...
        try:
            ...
            if live_stats_path.exists():
                ...
                n_skips = stats.get("n_skips", stats.get("n_filter_fail", 0))
                ...
                if "n_skips" in self.stats_labels:
                    self.stats_labels["n_skips"].setText(str(n_skips))
                used_th = stats.get("used_threshold")
                if "used_threshold" in self.stats_labels:
                    self.stats_labels["used_threshold"].setText(
                        f"{used_th:.3f}" if used_th is not None else "—"
                    )
                th_src = stats.get("threshold_source")
                if "threshold_source" in self.stats_labels:
                    self.stats_labels["threshold_source"].setText(str(th_src) if th_src else "—")
            ...
            if metrics_path.exists():
                ...
                n_skips = metrics.get("n_skips", metrics.get("n_filter_fail", 0))
                ...
                if "n_skips" in self.stats_labels:
                    self.stats_labels["n_skips"].setText(str(n_skips))
                used_th = metrics.get("used_threshold")
                if "used_threshold" in self.stats_labels:
                    self.stats_labels["used_threshold"].setText(
                        f"{used_th:.3f}" if used_th is not None else "—"
                    )
                th_src = metrics.get("threshold_source")
                if "threshold_source" in self.stats_labels:
                    self.stats_labels["threshold_source"].setText(str(th_src) if th_src else "—")
                if "max_drawdown" in self.stats_labels:
                    self.stats_labels["max_drawdown"].setText(f"{max_dd*100:.2f}%")
